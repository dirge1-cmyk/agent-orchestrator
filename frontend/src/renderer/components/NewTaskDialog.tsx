...
export function NewTaskDialog({ open, projectId, onCreated, onOpenChange }: NewTaskDialogProps) {
	...
	const projectQuery = useQuery({
		...
		queryFn: async () => {
			...
			if (data?.status !== "ok") throw new Error(t("newTask.errors.projectUnavailable"));
	...
	const submit = async (event: FormEvent<HTMLFormElement>) => {
		...
		if (!cleanTitle || !cleanPrompt) {
			setError(t("newTask.errors.required"));
			return;
		}
		...
		try {
			...
			if (apiError) throw new Error(apiErrorMessage(apiError, t("newTask.errors.startFailed")));
			if (!data?.session?.id) throw new Error(t("newTask.errors.noSession"));
			...
			setError(err instanceof Error ? err.message : t("newTask.errors.startFailed"));
		} finally {
			setIsSubmitting(false);
		}
	};
					...
					<form onSubmit={submit} className="space-y-4 px-5 py-4">
						...
						<div className="space-y-1.5">
							...
							<textarea
								...
								placeholder={t("newTask.fields.briefPlaceholder")}
								value={prompt}
						...
						<div className="grid gap-3 sm:grid-cols-[1fr_1fr]">
							<div className="space-y-1.5">
								<RequiredAgentField
									id={agentId}
									label={t("newTask.fields.agent")}
									placeholder={t("newTask.fields.projectDefault")}
									value={agent}
								...
								>
									{t(refreshAgentsMutation.isPending ? "newTask.refreshingAgents" : "projectAgents.refresh")}
								</button>
							</div>
							<div className="space-y-1.5">
								<Label className="text-xs font-medium text-muted-foreground" htmlFor={branchId}>
									{t("newTask.fields.branch")}
								</Label>
								<Input
									id={branchId}
						...
						<div className="flex items-center justify-end gap-2 pt-1">
							...
							<Button type="submit" disabled={isSubmitting || !projectId}>
								...
								{t(isSubmitting ? "newTask.starting" : "newTask.start")}
							</Button>
						</div>
					</form>
				</Dialog.Content>
