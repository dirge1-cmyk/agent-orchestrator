...
func initialPRStateChangedAt(r domain.PullRequest) time.Time {
	...
	case domain.PRStateMerged:
		return firstNonZeroTime(r.MergedAtProvider, r.UpdatedAtProvider, r.ObservedAt, r.UpdatedAt)
	case domain.PRStateClosed:
		return firstNonZeroTime(r.ClosedAtProvider, r.UpdatedAtProvider, r.ObservedAt, r.UpdatedAt)
	case domain.PRStateDraft, domain.PRStateOpen:
		return firstNonZeroTime(r.CreatedAtProvider, r.UpdatedAtProvider, r.ObservedAt, r.UpdatedAt)
	default:
		return firstNonZeroTime(r.ObservedAt, r.UpdatedAt)
	}
}
